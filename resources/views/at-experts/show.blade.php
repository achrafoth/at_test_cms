            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold">Expert Information</h3>
                        <div class="flex space-x-2">
                            <a href="{{ route('at-experts.edit', $atExpert) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                Edit
                            </a>
                            <a href="{{ route('at-experts.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                                Back to List
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg shadow-inner mb-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-base">{{ $atExpert->name }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Expertise Area</p>
                                <p class="text-base">{{ $atExpert->expertise_area ?? 'Not specified' }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Email</p>
                                <p class="text-base">{{ $atExpert->email }}</p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500">Phone</p>
                                <p class="text-base">{{ $atExpert->phone ?? 'Not specified' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-8">
                        ...
                        @if($atExpert->clients && $atExpert->clients->count() > 0)
                            <div class="overflow-x-auto">
                                <table class="min-w-full bg-white">
                                    ...
                                    <tbody>
                                        @foreach($atExpert->clients as $client)
                                            <tr>
                        ...
                        @else
                            <p class="text-gray-500">No clients assigned to this expert.</p>
                        @endif
                    </div>
                </div>
            </div>

                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('provisions.update', $provision) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="client_id" name="client_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client_id', $provision->client_id) == $client->id ? 'selected' : '' }}>
                                        {{ $client->full_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="at_equipment_id" name="at_equipment_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($equipment as $item)
                                    <option value="{{ $item->id }}" {{ old('at_equipment_id', $provision->at_equipment_id) == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="provision_date" class="block mt-1 w-full" type="date" name="provision_date" :value="old('provision_date', $provision->provision_date->format('Y-m-d'))" required />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="notes" name="notes" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('notes', $provision->notes) }}</textarea>
